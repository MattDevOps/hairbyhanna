    ...
    <section id={id} className="scroll-mt-28">
      <div className="card p-8">
        ...
        <div className="mt-6 grid gap-3">
          {items.map((it) => (
            <div key={it.name} className="flex items-start justify-between gap-6 border border-white/10 bg-white/5 rounded-2xl p-4">
              <div>
                <p className="font-semibold">{it.name}</p>
                {it.note ? <p className="mt-1 text-sm text-white/70">{it.note}</p> : null}
              </div>
              <span className="text-xs text-white/60 whitespace-nowrap">Pricing varies</span>
            </div>
          ))}
        </div>
      </div>
    ...
    <div className="mx-auto max-w-6xl px-6 py-16">
      ...
      <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-3 text-white/70 max-w-2xl">
            Pricing depends on hair length/density, timing, and your goals. We’ll confirm the range when you request your appointment.
          </p>
        </div>
      ...
      <div className="mt-10 grid gap-6">
        ...
        <Section
          ...
          items={[
            { name: "Brazilian Blowout", note: "Reduces frizz, improves manageability." }
          ]}
        />
